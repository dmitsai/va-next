...
import ChevroIcon from '~/shared/assets/icons/chevron-dark-icon.svg';
...
export const Select: React.FC = () => {
    ...
    return (
        <Listbox value={selected} onChange={setSelected}>
            <ListboxButton
                className={'relative flex flex-row max-w-52 w-full justify-between items-center group rounded-6 bg-mantle hover:bg-text py-1.5 pr-8 pl-4 text-left text-text hover:text-base font-400 text-14 leading-6 transition-colors'}
            >
                {selected}
                <ChevronLightIcon
            ...
            <ListboxOptions
                ...
                transition
                className={
                    'mt-2 w-[var(--button-width)] rounded-6 border-2 border-mauve bg-base p-2 [--anchor-gap:var(--spacing-1)] focus:outline-none transition duration-100 ease-in data-[leave]:data-[closed]:opacity-0'
                }
            >
                {state.map((item) => (
                    <ListboxOption
                    ...
                    >
                        ...
                        <div className="font-400 text-14 leading-5 text-text">{item}</div>
                    </ListboxOption>
                ))}
            </ListboxOptions>
        </Listbox>
