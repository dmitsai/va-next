...
const schema = z
    ...
    .refine(
        (data) => {
            const employmentTypesSelected = [
                'full',
                'partTime',
                'internship',
            ].some((key) => data[key as EmploymentTypesKeys] === true);
    ...
    .refine(
        (data) => {
            const workScheduleSelected = [
                'fullday',
                'shift',
                'flexible',
                'remote',
            ].some((key) => data[key as WorkScheduleKeys] === true);
...
const getEmploymentTypes = (
    ...
    return Object.entries(flags)
        ...
        .map(
            ([key, _value]) => employmentTypes[key as EmploymentTypesKeys]
        ) as Array<EmploymentTypes>;
};
const getSelectedWorkSchedules = (
    fullday: boolean,
    shift: boolean,
    flexible: boolean,
    remote: boolean
    ...
    return Object.entries(flags)
        .filter(([_key, value]) => value)
        .map(
            ([key, _value]) => workSchedule[key as WorkScheduleKeys]
        ) as Array<WorkSchedule>;
};
const parseForm = (data: FormData, selectedSalaryCurrency: SalaryCurrency) => {
    ...
    const selectedEmploymentTypes = getEmploymentTypes(
        full,
        partTime,
        internship
    );
    const selectedWorkSchedule = getSelectedWorkSchedules(
        fullday,
        shift,
        flexible,
        remote
...
export const EditProfilePreferences: React.FC<EditProfilePreferencesProps> = (
    props
) => {
    const {
        setIsOpen,
        employmentTypes: initEmploymentTypes,
        workSchedule: initWorkSchedule,
        salary,
        salaryCurrency,
    } = props;

    const initialEmployment = Object.entries(employmentTypes).map(
        ([key, value]) => [key, initEmploymentTypes.includes(value) ?? false]
    );
    const initialWork = Object.entries(workSchedule).map(([key, value]) => [
        key,
        initWorkSchedule.includes(value) ?? false,
    ]);
    ...
    useEffect(() => {
        console.log('ws', initialWork);
        console.log('et', initialEmployment);
    }, []);
    const {
        control,
        handleSubmit,
    ...
    const onSubmit = (data: FormData) => {
        ...
        const preferences = {
            workSchedule: parseData.workSchedule,
            employmentTypes: parseData.employmentTypes,
        };
        updateClient({
            preferences,
            currencyName: selected.title,
            salaryFrom: parseData.salary,
        });
        setIsOpen(false);
    };
        ...
        <Popup title={'Редактирование предпочтений'} {...props}>
            ...
            >
                <div className={'flex w-full flex-col gap-y-4'}>
                    ...
                    </span>
                    {
                        ...
                        employmentTypesArray.map(([key, value]) => (
                            <Checkbox
                                checked={formValues[key as EmploymentTypesKeys]}
                                key={key}
                                control={
                ...
                <div className={'flex w-full flex-col gap-y-4'}>
                    ...
                    </span>
                    {
                        ...
                        workScheduleArray.map(([key, value]) => (
                            <Checkbox
                                checked={formValues[key as WorkScheduleKeys]}
                                key={key}
                                control={
                                    control as unknown as Control<FieldValues>
                                }
                ...
                <Button
                    ...
                    disabled={!isValid || isClientPending}
                    className="bg-mauve text-14 text-base transition-colors hover:bg-text"
                >
                    {isClientPending ? (
