...
export interface EditProfileBioProps extends Omit<PopupProps, 'title'>{bio:ProfileBioProps}
...
export const EditProfileBio: React.FC<EditProfileBioProps> = (props) => {
    const { setIsOpen, bio:{ type, data} } = props;
    ...
    const { control, handleSubmit, formState: {isValid} } = useForm<PersonalInfoForm>({
    ...
    const onSubmit = (data: PersonalInfoForm) => {
        ...
        if (type === 'CLIENT'){
            console.log(data, 'data')
            updateClient({name: data.firstName, surname:data.lastName, patronymic: data.patronymic, telegram: data.telegram, phone:data.phone, email:data.email})
        }
        if (type === 'COMPANY'){
            updateCompany(data)
        }
    ...
    return (
        <Popup title="Редактирование персональной информации" {...props}>
            <form className={'flex flex-col gap-y-8 w-full items-end'} onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button type={'submit'} buttonView={ButtonView.LARGE} disabled={!isValid || isClientPending || isCompanyPending} className="w-1/4 text-base bg-mauve hover:bg-text transition-colors">
