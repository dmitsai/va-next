...
export const VacancyList: React.FC<VacancyListProps> = async (props) => {
  ...
  const vacancies = (await helpers.vacancy.infinityVacancy.fetch({ limit: 12 }))
    .vacancyList;

  const { resumeLabel } = props;
  return (
    <div className={'flex h-full flex-col items-center justify-center'}>
      <div className={'grid h-full w-full grid-cols-3 gap-5 xl:grid-cols-4'}>
        {resumeLabel ? (
          <span
            className={
          ...
          <span
            className={'col-span-3 mb-5 w-full text-18 text-text xl:col-span-4'}
          >
