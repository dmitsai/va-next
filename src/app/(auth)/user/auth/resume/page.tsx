...
const LoginPasswordPage = () => {
    ...
    const onSubmit = async (data: LoginPasswordFormData) => {
        ...
        await updateProfile({ name: data.firstName, surname: data.lastName });
        if (isValid) {
            router.push('/profile');
        }
    };
    ...
    return (
        <div className={`flex h-screen`}>
            <div className={`relative w-1/2 bg-mauve`}>
                <Link
                    className={`absolute left-10 top-10 text-16 text-base`}
                    href="/"
                >
                    {CONSTANTS.topBar.placeholder}
                </Link>
            </div>
            <div className={`relative w-1/2 bg-base`}>
                ...
                <Link className={`absolute left-10 top-8`} href="/user/auth">
                    <Button
                        type="button"
                        buttonView={ButtonView.LARGE}
                        className={`group bg-mantle text-text hover:bg-text`}
                    >
                        <BackIcon
                ...
                >
                    ...
                    >
                        <h1 className={`text-text`}>
                            {CONSTANTS.auth.lable.signUp}
                        </h1>
                        <p className={`text-16 text-text`}>
                            {CONSTANTS.auth.resume}
                        </p>
                    </div>
                    <form
                    ...
                    >
                        ...
                        <Button
                            ...
                            disabled={!!formState.errors.firstName || isPending}
                        >
                            {isPending ? (
